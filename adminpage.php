...
if(isset($_POST["submit1"])){
  ...
  if(mysqli_num_rows($duplicate) > 0){
    ...
    if($password == $confirmpassword){
      $query = "INSERT INTO mechanics (user_name, email, password, number, location)
      VALUES('$username','$email','$password', '$number', '$location')";
      $res=mysqli_query($conn,$query);
        if($res){
          //echo "Data inserted successfully";
          header("Location:display.php");
  ...
  <body>
    <header class="header">
      ...
      <nav class="navbar">
        ...
        <a href="mechs.php">View mechanics</a>
    ...
    <form class="register-form" action="" method="post" autocomplete="off">
    <h3>Add Mechanic</h3>
      <label for="user_name">Mech Name:</label>
      <input type="text" name="user_name" id="user_name" required> <br>
      <label for="email">Email : </label>
      <input type="email" name="email" id = "email" required value=""> <br>
      ...
      <label for="confirmpassword">Confirm Password : </label>
      <input type="password" name="confirmpassword" id = "confirmpassword" required value=""> <br>
      <button type="submit1" name="submit1">Add</button> <br>
